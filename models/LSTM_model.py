...
def lstm(data, loc=0.0):
    ...
    test_generator = TimeseriesGenerator(scaled_X_test, np.zeros(len(X_test)), length=1, batch_size=1)
    ...
    # Model
    model = Sequential()
    model.add(LSTM(128, activation='relu', input_shape=(n_input, n_features), dropout=0))
    model.add(Dense(1))

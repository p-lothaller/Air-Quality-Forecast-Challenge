...
def lstm(data, loc=0.0, dropout=0.1):
    ...
    model_checkpoint = ModelCheckpoint(filepath = './B-PM25-{epoch}-{loss:.2f}.h5', monitor='loss', save_best_only=True, verbose=1)
    ...
    model10 = model
    early_stopping = EarlyStopping(monitor='loss', patience=5)
    model_checkpoint = ModelCheckpoint(filepath = './B-PM10-{epoch}-{loss:.2f}.h5', monitor='loss', save_best_only=True, verbose=1)
